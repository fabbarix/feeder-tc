...
Last updated: 2026-08-23 (eight packages merged 22–23 Aug — see the integration log)
...
| 2026-08-22/23 | UA review round + 1.0 scope | **Eight packages merged, `4dd8818` → `90093ce`.** Verified locally on merged `main` after each, per WP-01's `pull_request`-only CI. Final: lint/typecheck/build green, **1319 tests**, **331 E2E**. Sequence: shopping "why" per row + pantry History refresh (`4dd8818`); the five defects three no-docs usability reviewers found (`2c323ce`); the card grid past 1440px (`5f49d3b`); leftover-aware planning (`0f9cd85`); the product re-key (`3c3b51e`); the token layer (`e84190d`); desktop density (`604de7e`); the products screen and its three price views (`90093ce`). |
| 2026-08-23 | — | **Two defects nobody reported, found by measuring rather than reading.** `ConfirmDialog`'s destructive button was hardcoded `#fff` and had only ever been checked in light mode; in dark mode `--danger`'s fill is light, so it measured **2.62:1** — failing WCAG AA on the confirm button for destructive actions. And Plan's view switcher had been rendering as a wrapped 999px pill (a blob) since `07aa2ea`. **The coordinator's own conformance test missed the second one** because it pinned three specific consumers — theme control, household flag, pantry rail — instead of the invariant. Replaced with a sweep asserting no segmented control renders multiple rows while carrying the pill radius, across every consumer at every tier. Standing lesson: pin the property, not the instance. |
| 2026-08-23 | — | **Three claims from reviewers were disproved by measurement, and one of the coordinator's own.** Reported and refuted: toasts "never auto-dismiss" (gone by 5s on a wall clock); the pantry Location filter "renders outside its pill" (measured `inline-grid`, clean 2×2, no overflow); "Generate week overwrote a deliberately chosen meal" (the pick survived — the real defect was that generation filled 3 slots of 20 and said nothing). The coordinator also wrongly attributed the pill blob to larger type; an agent measured the prior commit and proved it predated the token work. Screenshots produced all four errors; measurement settled all four. |
